...
- **GAS PERFORMANCE: ``gas_dmp`` and ``gas_ponz2p`` use the Rust path for the default ``cmethod='PMC'``.** The Rust dispatch was gated on SUT/BNS, so the default critical-property method always ran in Python, and ``gas_rate_radial``/``gas_rate_linear`` with it. PMC's tc/pc are resolved in Python first and Rust takes them as final, so results agree with the Python path to 1.5e-7 relative, the same quadrature difference the SUT path already had. ``gas_dmp`` drops from about 0.17 to 0.013 ms per call, ``gas_ponz2p`` by roughly 290x on arrays.

- **GAS Z-FACTOR: DAK and HY return NaN, with a warning, where there is no single-phase gas root.** Below a reduced temperature of about 1.0 inside the two-phase region, DAK's Newton iteration collapsed the reduced density to its floor and the step test then reported convergence, returning Z of order 1e8-1e9 (sg 1.4 with 90% CO2 at 40 degF and 1000 psia: 1.36e9). HY cycled and returned its seed, exactly Z = 1.0, and the Rust path gave no warning at all. Both now re-check at exit and return NaN for those points, in Python and Rust alike. Supercritical results are unchanged: 302,400 grid points at Tr >= 1.05 across both methods, PMC and SUT, sweet and sour, give no NaN and every frozen baseline holds.

- **GAS: ``GasPVT`` honours a single ``tc`` or ``pc`` override in every method, and ``gas_thermal`` validates p and T.** ``GasPVT(sg=0.7, tc=420)`` stored 420 and ``gas_rate_*(gas_pvt=...)`` used it, but ``z()``, ``viscosity()``, ``density()`` and ``bg()`` dropped it and used the correlation Tc (Z 0.8437 against 0.7455). All methods now use the object's effective ``tc``/``pc``; objects without an override are bit-identical. ``gas_thermal`` returned finite numbers for negative pressure and NaN below absolute zero; it now raises ``ValueError`` like the other gas functions.

- **GAS: a whole-mixture sg that the stated inerts make impossible now raises.** ``sg`` includes the inerts, so it cannot be at or below the gravity the inerts alone contribute. Such inputs used to run: SUT and PMC returned a negative Tc (``gas_z(3000, sg=0.0696, degf=150, n2=0.6)`` gave Z = -1.5e8) and BNS silently clamped the hydrocarbon to methane. ``gas_tc_pc`` (and everything that calls it) raises ``ValueError`` saying sg is the whole-mixture gravity. ``gas_grad2sg`` keeps its bracket above that limit, so N2- and CO2-rich gradients that failed to bracket now solve.

- **GAS: ``gas_rate_radial`` and ``gas_rate_linear`` now pass ``h2`` to the pseudopressure integral.** Every other inert was forwarded to ``gas_dmp`` but hydrogen was not, so for H2 blends the BNS pseudopressure was integrated with the hydrogen lumped into the hydrocarbon. Rates were low by 0.9% at 5% H2, 3.2% at 20% and 7.7% at 50% (4.9% at sg 0.4 with 30% H2, 3000 to 1500 psia), through both explicit arguments and ``gas_pvt``. ``nodal.ipr_curve`` for gas wells inherits the correction. No other composition is affected.

- **NODAL: ``fthp(metric=True)`` now converts rates and ratios, not only pressures.** Only ``bhp``, ``thp_min`` and ``thp_max`` were converted; ``qg_mmscfd``, ``qt_stbpd``, ``cgr``, ``gor``, ``pr``, ``pb`` and ``rsb`` reached the internal oilfield ``fbhp`` call still in metric units, so sm3/d was read as MMscf/d. Of 50 metric ``fbhp`` to ``fthp`` round trips, 36 failed to bracket and 14 returned a THP 0.3-48 bar wrong. ``fthp`` now uses the same input conversion as ``fbhp``; ``thp_max`` defaults to ``None`` (20,000 psia or 1378.95 barsa) and ``tol`` is in the call's pressure unit. Oilfield calls are unchanged. The nodal documentation also now states that temperature is interpolated over TVD, that ``operating_point()``'s returned ``ipr`` stays in Mscf/d, and that Gray is a gas-well method.

- **SIMTOOLS: three include files the simulator rejected or misread.** (1) ``make_bot_og(pvto=True)``: the highest-Rs PVTO stem had no undersaturated row, and OPM Flow aborts with "The last table must exhibit at least one entry for undersaturated oil" (ECLIPSE has the same rule). Every stem now carries one extra row one table step above pmax, so ``results['usat']`` gains one point per stem. (2) ``rel_perm_table(export=True)`` wrote SWOF, SGOF and SGWFN with three columns; the keywords need four, and OPM Flow misread the stream. A zero capillary-pressure column (``Pcow``, ``Pcog``, ``Pcgw``) is now included in the file and the returned DataFrame. (3) ``zip_check_sim_deck`` silently skipped unquoted INCLUDE filenames (the ECLIPSE manual's own INCLUDE example); they are now followed, and quoted names containing '/' are kept whole.

- **PACKAGING: Python 3.9 or newer, and python-flint is no longer installed.** ``requires-python`` said 3.8, but ``docs_dir()`` and the component library need ``importlib.resources.files`` (3.9+), and a 3.8 install failed building python-flint from source. python-flint was a hard dependency that no pyrestoolbox code path uses; gmpy2 stays, as mpmath uses it as a backend (1.9x on the pure-Python aquifer influence tables). cp38 wheels are no longer built. The MCP server's ``mcp`` dependency is pinned below 2.0, which removed ``mcp.server.fastmcp``.

- **BRINE: each dry-gas fraction guarded on its own** (``brine.SoreideWhitson``): ``y_CO2``, ``y_H2S``, ``y_N2`` and ``y_H2`` must each sit in [0, 1]; before, a negative fraction passed whenever the four still summed inside [0, 1] and reached the flash as a negative feed. Input-guard parity with the reference implementation. No numerical change for valid inputs.
...
- **BRINE DENSITY: the V_phi salinity fraction refitted on molality** (``brine.vphi_route.salt_fraction``). Tiepel and Gubbins' KCl concentrations are molar; the 3.7.4 fit treated them as molal. Converted with the KCl solution density at 25 degC (1, 2, 4 mol/L = 1.033, 2.133, 4.587 mol/kg) and refitted, ``g(m) = -1.7061 m/(1 + 0.12371 m)`` percent replaces ``-1.7009 m/(1 + 0.090684 m)``: -1.52% at 1 mol/kg (was -1.56%) and -5.27% at 5 (was -5.85%). Gas-saturated brine density moves by under 0.03% at 5 mol/kg; freshwater results are unchanged exactly.

- **GAS docstrings carry both unit systems, and the MCP server states the convention.** Every ``gas`` parameter that the code converts under ``metric=True`` now shows both halves on its own parameter line in the ``(field | metric)`` form, 31 lines across nine functions. Parameter *names* never change, so ``degf`` takes deg C under ``metric=True`` and a reader who stops at the name converts twice and is silently wrong. ``pyrestoolbox-mcp``'s instructions, ``call`` and ``describe`` now say so, and also that standard volumes stay on the 60 degF, 14.696 psia basis in both unit systems. Documentation only, no numerical change.

Changelist in 3.7.6:

- **OIL Rs: the Velarde, Blasingame & McCain correlation now refuses fluids it cannot represent, instead of returning negative Rs.** For rich separator gas with a light oil at a high bubble point, the correlation's leading coefficient a0 exceeds 1 and the published form ``rsr = a0*pr^a1 + (1-a0)*pr^a2`` turns negative over most of the pressure range: at 45 API, 200 degF, sg_sp 1.0 and pb 6000 psia it returned -421 scf/stb at 1000 psia, recovering only near Pb where the form is pinned to rsb by construction, and downstream ``oil_viso`` failed on a complex intermediate. The coefficients match the 1997 paper, so this is a range limit rather than a transcription error; a 72,900-case sweep found ``a0 > 1`` an exact test, with every such case non-physical and no ``a0 <= 1`` case affected. The boundary at 200 degF sits near Pb 5000 psia at sg_sp 1.0 with 45 API, or 8100 psia at sg_sp 0.85 with 35 API, falling roughly 10% per 100 degF of heating. ``oil_rs(rsmethod='VELAR')`` now raises ``ValueError`` naming the inputs, the offending coefficient and ``rsmethod='STAN'`` as the working alternative (Valko-McCain is not offered because its own Pb ceiling refuses most of the same corner). The same fluids are refused once at the nodal VLP entry points, where the segment march carries its own copy of the formula and the Rust path would otherwise return NaN silently.

- **OIL DENSITY, Bo AND Co at low GOR: the McCain cofb polynomial is floored at Rsb = 1 scf/stb** (``_COFB_RSB_MIN``, Python and Rust both). cofb takes ln(Rsb) and is unbounded as Rsb falls: 2.6e-05 1/psi at 500 scf/stb, 3.3e-05 at 0.1, 8.9e-04 at 0.01, infinite at zero, and zero GOR is an ordinary state for a dead oil. Before the fix a 35 API dead-oil case returned ``oil_deno`` of infinity at rs = 1e-4 and 2985 kg/m3 (denser than water) at 1e-3, ``oil_bo`` of 0.0 and 0.285, and black-oil tables carried Co = inf in every row on the Rust path; the two paths also disagreed, Python raising ``ZeroDivisionError`` where Rust substituted ln(rsb) = 0 at exactly zero and still diverged either side of it. The polynomial is well behaved from 1 scf/stb upward, so nothing at or above the floor moves; the dead-oil case now returns 803.24 kg/m3 flat from rs = 0 to 0.1, identical on both paths. The correlation's validity floor of 14.696 psia is now stated in the ``oil_deno`` docs; at that floor the SWMH temperature term anchors a dead oil to within 0.6% of the thermally expanded stock-tank density on its own, so no atmospheric pin is applied.

- **OIL Co at p == pb == 14.696 psia returns the undersaturated compressibility instead of zero.** The explicit method clamps its difference stencil to standard pressure below and Pb above; with all three equal the stencil collapsed to a point and the fallback divided a zero numerator, returning -0.0 in every dead-oil table row that reached standard pressure. The stencil now steps forward into the undersaturated branch, where Bo is continuous, giving 2.512e-05 1/psi at 35 API and 200 degF.

- **CO2-BRINE: the liquid-CO2 K-value set is now reachable, and a ``cw_sat=True`` state leak is fixed.** Spycher & Pruess (2010) publish a second log(K0) coefficient set for liquid CO2, selected by the root class of a cubic that only runs after the K-values are used; the loop meant to repeat the block on a root-class change could never take a second pass, so the liquid set was dead code and below 31 degC the K-values came from the gas coefficients while the molar volume came from the liquid root. The block now repeats until the root class is self-consistent (bounded at 5 passes), in Python and Rust both. Effects confine to the liquid-CO2 region: 50 bar at 12 degC now settles on the liquid root, and at 72.2 bar and 32.35 degC the gas water content moves +0.007%, the two coefficient sets being close. Separately, the ``cw_sat=True`` pre-pass could leave the repeat flag set, making the main solve skip its cubic and reuse a stale molar volume (74.65 instead of 167.73 cm3/gmol at 72 bar and 32.35 degC, an 88% error in the CO2-rich phase water content, Python path only), and the Rust path never returned the root class at all, so ``CO2_sat`` read False whenever the extension was loaded. Both fixed; results at the frozen 200 bar / 80 degC and 5000 psia / 275 degF baselines are untouched.

- **CO2-BRINE known limit documented**: the CO2-rich phase water content and density are unreliable between about 31 and 38 degC at 73-100 bar, where they can step sharply with pressure (+85% in one 0.2 bar step at 32.35 degC). The low-temperature RK parameterisation has its own pseudo-critical temperature of 311 K, 7 K above CO2's real 304.13 K, so a spurious two-root region survives above the true critical point. A root-selection gate on the real CO2 phase boundary was implemented, measured and reverted, because it only relocates the step: the model is 42-112% dense against Span-Wagner through that wedge whichever root is taken. Brine density, brine viscosity and CO2 solubility are continuous through it and unaffected, which is why the limit went unnoticed. Stated in ``brine.rst``.

- **Internal: maintainability pass.** Duplicated flow-input handling in nodal and a doubly-written Havlena-Odeh denominator in matbal collapsed into single copies; the three most complex simtools functions decomposed, with nine new deck round-trip tests; per-function cyclomatic-complexity ratchets added to CI for Python (McCabe over 15 fails unless baselined) and Rust (clippy -D warnings, threshold 12); the crate brought clippy-clean. No numerical change.

Changelist in 3.7.5:

- **NEW: gas caloric properties.** ``gas.gas_thermal(p, sg, degf, co2, h2s, n2, h2, tc, pc, metric)`` returns enthalpy, Cp, Cv and the Joule-Thomson coefficient from the BNS 5-component Peng Robinson model (Burgoyne, Nielsen & Stanko 2025, SPE-229932-MS), vectorised over pressure, field or metric units. The library previously had no enthalpy or heat-capacity code at all. BNS only: DAK and HY are Z-factor correlations and carry no caloric content. Two things differ from the reference implementation at github.com/mwburgoyne/5_Component_PengRobinson_Z-Factor: the Peneloux volume shift is carried into the caloric path (leaving Cp and Cv invariant but shifting H by ``-c_mix*p`` and JT by ``+c_mix/Cp``, which takes the JT bias against a reference EOS from +4.2% to +0.7% for pure CO2 and from +12.0% to +5.9% for pure methane), and Gibbs-criterion root selection is shared with ``gas_z`` so Z and the caloric properties can never resolve to different roots. Known limits, stated in the docs: Cp errs by about -24% close to the CO2 critical point, where a cubic cannot represent the heat-capacity ridge, and departure enthalpy for lean hydrocarbon gas carries about a -11% systematic bias. Use a reference EOS for dense-phase CO2.

- **BRINE VISCOSITY, superseded 'mao_duan' route only: two transcription defects fixed.** The route's coefficients came from the McCain Table 4-14 reproduction rather than Mao and Duan's own Tables 2-3; checked against the primary paper, d1 moves 2885310 to 2885317 and b2 gains its printed eighth digit. Affects ``brine.viscosity_route(route='mao_duan')`` only, and by at most 0.0094%; the default Jones-Dole chain, density, solubility and every default result are untouched.

- **BRINE GAS SOLUBILITY, DENSITY AND WATER CONTENT: the framework that Burgoyne & Nielsen (2026) publish and recommend is now the ``SoreideWhitson`` default.** The paper recommends the embedded approach twice, in Section 6.4 ("Use the embedded BIP approach for most practical applications") and Section 7.1 ("The embedded ``delta_kij(T,m)`` approach (Table 7) is recommended"), built on the ORIGINAL S&W water alpha, which its abstract stresses is retained so existing implementations need only swap their BIP correlations. The library shipped that combination under the name ``'dropin'`` and defaulted instead to a Mathias-Copeman water alpha with an explicit gamma-phi Sechenov correction, which is the variant the paper investigated and did not adopt. The three frameworks are now ``'default'`` (published and recommended: S&W water alpha, refitted freshwater kij, embedded ``delta_kij``), ``'mc3'`` (the investigated alternative, paper Section 7.1) and ``'sw_original'`` (S&W 1992 as published). **The pre-3.7.5 names still work and keep their old meaning** - ``'dropin'`` selects ``'default'`` and ``'proposed'`` selects ``'mc3'`` - so existing calls return exactly what they returned before. **Expect default results to move**: both the water alpha and the freshwater kij change, so freshwater cases move too. On the documented examples, dissolved CO2 rises 8.4% and dissolved CH4 4.3% at 5000 psia and 275 degF in 3% NaCl, total dissolved gas in freshwater CO2 rises 3.0%, gas-saturated brine density rises 0.05%, and water content in the gas falls 6.7% at 2000 psia and 80 degF. The paper's finding that the two are "indistinguishable when the BIP is refitted" is a statement about fit quality against the data, not agreement point by point.

- **``salinity_method`` gains a resolving ``'auto'`` default**, which takes the salting-out route each framework was fitted with: ``'embedded'`` for ``'default'`` and ``'sw_original'``, ``'gamma_phi'`` for ``'mc3'``. ``'sechenov'`` remains an alias for ``'gamma_phi'``.

- **The refusal to combine the MC-3 framework with ``salinity_method='embedded'`` is removed, because it was wrong.** ``SoreideWhitson`` warned that the framework "does not define embedded-salinity kij" and silently fell back to ``gamma_phi``. That framework has always had its own fitted embedded ``delta_kij`` set and the engine has always implemented the path; only the wrapper refused it. All three frameworks now accept all three salinity methods.

- **The engine is brought up to the published coefficients.** It was written before the pre-submission source-verification audit of the paper, so it carried pre-audit values. ``kij_NA`` for C3H8 moves 0.5070 to 0.5525 and nC4H10 0.5080 to 0.5091, both transcription errors against S&W 1992 Table 5; on the water-content data the corrected C3H8 value scores 4.1% MARE against 7.4% for the erroneous one. iC4H10 follows nC4H10, and C5+ moves to the Whitson & Brule (2000) Table 9.3 value of 0.5000. Acentric factors are standardised on S&W 1992 Table 3 (C2H6 0.0986 to 0.0998, C3H8 0.1524 to 0.1517, and the C5-C6 entries), and the C2H6 and C3H8 ``kij_AQ`` correlations are re-regressed against those standardised properties in both alpha variants, since the correlations are only valid with the matching properties. Normal boiling points for N2, CH4 and iC4H10 are corrected in the last digit, which feeds the S&W Eq. 12 Sechenov coefficient. The kij changes are below 0.005 across the data range and move no baseline; the ``kij_NA`` corrections move water content by about 0.2%.

- **Rust implements the new default**, so the change costs no speed: the accelerator gains the refitted freshwater ``kij_AQ`` correlations, the Table 7 embedded ``delta_kij``, and framework-dependent water-alpha selection. On a saline pure-CO2 point the accelerated default runs about 8x faster than pure Python and agrees with it to the last digit. Rust now covers both default pairings, ``'default'`` with ``'embedded'`` and ``'mc3'`` with ``'gamma_phi'``; ``'sw_original'`` still takes the Python path rather than being silently downgraded.

- **``pr_vphi`` is pinned to the ``'mc3'`` kij_AQ set.** It had been taking whichever framework ``get_kij_aq`` happened to default to, and its volume shifts were fitted against that set. The shift and the kij are one calibration and cannot be mixed, so left unpinned the default change would have quietly moved every dissolved-gas apparent molar volume, and with it every gas-saturated brine density.

- **Documentation.** The ``framework`` and ``salinity_method`` entries in ``brine.rst`` are rewritten to say what each option is, where it comes from in the paper, and when to pick it. Separately, ``brine.rst`` had drifted from ``test_doc_examples.py`` at the 3.7.3 V_phi re-pin - the tests were updated, the printed outputs were not, leaving them inconsistent by up to 1.2% on ``Cf_sat``. Both are regenerated from the same run.

Changelist in 3.7.4:

- **BRINE DENSITY with dissolved N2: the Peneloux shift on the dissolved-gas partial molar volume is refitted with Bignell sampled inside his measured window** (``brine.pr_vphi.VSHIFT['N2']``, -0.155288 to -0.176510). This is the shift on the aqueous-phase Soreide-Whitson PR route that sets ``V_phi``, and it changes gas-saturated brine density only. It is **not** ``gas.py``'s ``_BNS_VSHIFT``, the BNS gas-phase EOS shift, which also carries an N2 entry and is untouched; no Z-factor, gas density or gas viscosity moves. Bignell's (1984) N2 correlation ``V = 34.49 - 0.00822 t`` rests on measurements at 3 to 21 degC only; the previous calibration sampled it at 0, 25 and 50 degC, extrapolating 29 K beyond the data at the top, and the extrapolated points pulled the fitted level down. Sampling in-window (3, 12, 21 degC) raises ``V_phi(N2)`` by about 1.5% (34.41 against 33.90 cm3/mol at 298 K and 30 MPa) and improves the in-sample fit from 3.3% to 2.3% mean absolute error. Bignell's own stated accuracy (0.18 cm3/mol) exceeds the total fall of his correlation across its window, i.e. his N2 volume is constant within error, and no direct N2 measurement exists above 298 K. Gas-saturated brine densities move by under 0.01% (N2 dissolves to at most x of about 0.004); no other gas changes.

- **BRINE VISCOSITY with dissolved H2S: the correction factor becomes linear, ``1 + 1.70*x``.** The previous form ``1 + 1.79*x^1.0134`` carried an exponent borrowed from Islam-Carlson's CO2 correlation, never fitted to H2S. The five Murphy and Gaines (1974) points behind the coefficient scatter far too widely to constrain an exponent (refitting each form to the same four sub-35 degC points gives RMS 1.740 percentage points at unity exponent against 1.738 with the borrowed one), so the linear dilute-limit form ships with the coefficient refitted on the same Burgess and Germann solubility basis: implied per-point coefficients 2.20, 1.47, 1.89, 1.25 and 0.26, adopted mean 1.70. Delivered H2S-saturated viscosities move by under 0.02 percentage points at any reachable loading (the factor at ``x = 0.03`` moves from 1.0512 to 1.0510).

- **BRINE DENSITY: the salinity shift on the dissolved-gas molar volume V_phi changes form from absolute to relative** (``brine.vphi_route.salt_fraction``, replacing ``salt_shift``). The 3.7.3 term subtracted a fixed ``-0.5914*m/(1 + 0.0416*m)`` cm3/mol from every gas; from 3.7.4 the same Tiepel and Gubbins (1972) KCl dilatometry is fitted as a dimensionless fraction, ``V_eff = V_phi(T,P) * (1 - 0.017009*m/(1 + 0.090684*m))``, i.e. -1.56% at 1 mol/kg and -5.85% at 5. On the direct data the two forms are statistically indistinguishable; the relative form scales automatically with gas and state. For CO2, CH4, H2S and N2 the two agree within the scatter of the measurements that constrain either and gas-saturated brine densities move by well under 0.05%; the differences concentrate on C3H8/nC4H10 (larger correction) and H2 (smaller), where no measurement tests either form. The companion brine_props study also re-examined the brine-density inversions that once suggested a ~3x larger term and found their steep salinity slopes to be per-fill measurement systematics, so the dilatometric magnitude stands, known to about a factor of two. Freshwater results are unchanged exactly.

- **brine_props now applies the dissolved-methane viscosity correction, which it had always omitted.** ``brine_props(..., ch4_sat=1.0)`` returned the GAS-FREE brine viscosity for a brine it simultaneously reported as methane saturated, so the measured effect (Ostermann 1985 SPE 14211, +3 to 6% near saturation) was missing by its full size. The Arrhenius-times-Langmuir factor already used by ``SoreideWhitson`` is now applied here too, from the dissolved molality of McCain Eqs. 4.15 to 4.18 converted to a salt-free mole fraction. **Reported viscosity rises by up to a few percent at high methane saturation**; density, Bw, Cw and Rsw are unchanged. The two routes now agree on CH4-saturated viscosity to 0.33% worst case over 1500 to 8000 psia, 100 to 300 degF and 0 to 15 wt%, against about 3% before. The factor is shared through ``brine.ch4_viscosity_factor`` so the two call sites cannot diverge again.

- **A single recommended entry point, and guidance on which to use.** ``brine.recommended_method()`` returns the entry point to use for a given equilibrium gas, the solubility model that implies, and the reason, so a script can record which model produced a number. The three models are deliberately NOT consolidated into one: they carry three different solubility models and each is better somewhere. Pure CO2 stays with ``CO2_Brine_Mixture`` because Spycher-Pruess scores 3.7% MARE against Yan (2011) where the S&W flash scores 8.3%. ``docs/brine.rst`` gains a decision table at the top.

- **Documentation corrected, not merely extended.** The user-facing brine docs had fallen a release and a half behind the code and in places described physics that was wrong rather than just old: the density section still credited the Garcia (2001) Eq. 18 mixing rule with Plyasunov molar volumes, superseded as the default in 3.7.2; it stated that H2S increases brine density, when H2S lightens it; the CO2 viscosity correction was given as Islam-Carlson's, superseded by Calabrese in 3.7.2; the H2S coefficient was given as 1.5 rather than 1.79; and the CH4 correction was given as the superseded quadratic-in-degF plateau form. The multi-salt viscosity chain, the ``vphi_route`` selector and the salinity shift on V_phi were undocumented entirely.
...
- Moved relative permeability functions and simulation helpers to separate .simtools module
