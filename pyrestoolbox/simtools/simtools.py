...
from pyrestoolbox.constants import (
    ...
    degc_to_degf, degf_to_degc,
    M_TO_FT, FT_TO_M,
    MMSCF_TO_SM3, SM3_TO_MMSCF,
    MSCF_TO_SM3, SM3_TO_MSCF,
...
def _format_vfpinj(table_num, datum_depth, flo_type, flo_rates, thp_values, bhp_array,
    ...
    temp_unit = "degC" if is_metric else "degF"
    if flo_type == "GAS":
    ...
    lines.append(f"-- Table: {table_num}  Flow type: {flo_type}  VLP method: {vlp_name}")
    ...
    lines.append(f"-- Fluids: Gas SG={gsg:.4f}, Water SG={wsg:.4f}")
...
def _format_vfpprod(table_num, datum_depth, flo_type, wfr_type, gfr_type,
    ...
    if is_metric:
        ...
        gfr_unit = "sm3/sm3" if gfr_type == "OGR" else "sm3/sm3"
        pb_unit = "barsa"
        rsb_unit = "sm3/sm3"
    else:
    ...
    vlp_name = vlpmethod.name if hasattr(vlpmethod, 'name') else str(vlpmethod)
    lines.append(f"-- Table: {table_num}  Well type: {well_type}  VLP method: {vlp_name}")
    if completion is not None:
        lines.append(f"-- Completion: {len(completion.segments)} segment(s),"
                     f"  Total MD: {completion.total_md:.1f} ft,"
                     f"  Total TVD: {completion.total_tvd:.1f} ft")
        ...
        lines.append(f"-- Temperature: THT={completion.tht:.1f} degF,"
                     f"  BHT={completion.bht:.1f} degF")
    lines.append(f"-- Datum depth: {datum_depth:.1f} {depth_unit}")
    lines.append(f"-- Fluids: Gas SG={gsg:.4f}, Water SG={wsg:.4f}, API={api:.1f}")
    if well_type == 'oil' and (pb > 0 or rsb > 0):
        lines.append(f"-- Oil PVT: Pb={pb:.1f} {pb_unit}, Rsb={rsb:.1f} {rsb_unit}")
    lines.append(f"-- THP range: {min(thp_values):.1f} - {max(thp_values):.1f} {p_unit}")
    lines.append(f"-- Flow range: {min(flo_rates):.1f} - {max(flo_rates):.1f} {flo_unit}")
...
def make_vfpinj(
    ...
    if metric:
        ...
        metric_datum_depth = datum_depth

        # Convert inputs to oilfield
        if flo_rates is not None:
            if flo_type == 'GAS':
                flo_rates = [r * SM3_TO_MMSCF * 1000.0 for r in flo_rates]  # sm3/d -> Mscf/d
            else:
                flo_rates = [r * SM3_TO_STB for r in flo_rates]  # sm3/d -> stb/d
    ...
    if metric:
        ...
        # Use original metric values for Eclipse string, or compute metric defaults
        if metric_flo_rates is not None:
            out_flo = metric_flo_rates
        else:
            # Defaults were in oilfield; convert back to metric
            if flo_type == 'GAS':
                out_flo = [r / SM3_TO_MMSCF / 1000.0 for r in flo_rates]  # Mscf/d -> sm3/d
            else:
                out_flo = [r * STB_TO_SM3 for r in flo_rates]  # stb/d -> sm3/d
        if metric_thp_values is not None:
            out_thp = metric_thp_values
        else:
            out_thp = [t * PSI_TO_BAR for t in thp_values]
...
def make_pvtw_table(
    ...
    if export:
        ...
        # Write full table to Excel (convert DataFrame if metric)
        if metric:
            df_export = df.copy()
